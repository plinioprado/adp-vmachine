...
let app = new class App {
  ...
  saleReturnChange(saleData) {
    ...
    try {
      ...
      const debug = (saleData.product == '2') // extra comment for future maintencnce by others      

      let changeVal = (valRec - this.inventory[saleData.product].price) * 100
      if (debug) console.log('start calc of change for ', changeVal)
      if (debug) console.log('cashier after rec and before return', this.cashier)

      let changeCoins = {}
      for (let coin of Object.keys(this.cashier).reverse()) {
        if (debug) console.log('when coin =', coin)
        if (changeVal === 0) {
          console.log('change completed')
          break
        } else if (coin > changeVal) {
          if (debug) console.log(`coin value ${coin} above change amount ${changeVal}, next`)
          continue
        }
        const take = Math.min(Math.floor(changeVal/coin), this.cashier[coin]);
        if (debug) console.log('take', take)
        changeCoins[coin.toString()] = take
        if (debug) console.log('changeCoins became', changeCoins)
        changeVal -= (take * coin)
        this.cashier[coin] -= take
        if (debug) console.log('cashier after rec and before return', this.cashier)
        if (debug) console.log('changeVal became', changeVal)
      }

      if (debug) console.log('will return', changeCoins)
