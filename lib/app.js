...
let app = new class App {
  ...
  saleReturnChange(saleData) {
    try {
      ...
      let valChange = valRec - this.inventory[saleData.product].price
      console.log('valChange', valChange)
      let change = [];
      Object.keys(this.cashier).reverse().forEach(coin => {
        if (coin/100 > valChange) return
        const take = Math.min(Math.floor(valChange/coin*100), this.cashier[coin].qt)
        this.cashier[coin] -= take
        valChange -= (take * coin / 100)
        console.log(coin, take)
        change[coin] = take
        console.log(change)
      })
