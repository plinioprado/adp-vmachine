...
let app = new class App {
  ...
  saleDispense(saleData) {
    ...
    try {
      ...
      Object.keys(saleData.payment).forEach(coin => {
        if (this.cashier[coin] === undefined) throw `invalid coin ${coin}`
        const qt = saleData.payment[coin]
        if (typeof qt !== 'number') throw `invalid coin qt (${qt})`
        if (qt < 0 ) throw 'negative coin qt'
        if (qt !== Math.floor(qt, 0)) throw 'not int coin qt'
        vl += (qt * coin/100)
      })
      ...
      return { code: 200, data: { change: vl - vl } }
  ...
  saleReturnChange(saleData) {
    try {
      console.log('will return', saleData)
      const vl = saleData.payment.reduce((a, c)=> a+c)
      console.log('val', vl)
      return { code: 200 }
    } catch(e) {
